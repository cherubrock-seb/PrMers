...
case "$TARGET" in rtx3080|radeonVII) ;; *) echo 'Use AEVUM_TARGET=rtx3080 or radeonVII' >&2; exit 2;; esac
OUT="$(mktemp -d "$ROOT/pass5-hwfix-${TARGET}-XXXXXXXX")"
...
python3 "$ROOT/scripts/pass5_validate.py" focused "$ROOT" "$OUT" "$DEVICE" "$TARGET"
