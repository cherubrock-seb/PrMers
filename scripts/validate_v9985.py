...
VERSION = "4.20.74-alpha-v99.85-stable-release-plan-audit"
ZIP_NAME = "PrMers-v99.85-StableReleasePlanAudit.zip"
...
def common_cases(target: str, profile: str) -> list[Case]:
    ...
    if target == "ubuntu":
        cases += [
            Case("prp-large-auto-type4", 175000039, 50,
                 ["175000039", "-prp", "-proof", "0"],
                 [r"\[Backend Auto\] PRP: Aevum selected", r"family=Type4 FFT323161", r"Progress:"],
                 fatal, smoke, "Selection/progress smoke; timeout is expected"),
            Case("ll-large-auto-type4", 175000039, 50,
                 ["175000039", "-ll"],
                 [r"\[Backend Auto\] LL: Aevum selected", r"family=Type4 FFT323161", r"Progress:"],
                 fatal, smoke, "Selection/progress smoke; timeout is expected"),
            Case("ecm-auto-type1", 55050557, 150,
                 ["55050557", "-ecm", "-b1", "50", "-K", "1", "-seed", "123456789", "-ecm_progress_ms", "500"],
                 [r"\[Backend Auto\] ECM: Aevum selected", r"family=Type1 FFT3161", r"Stage1"],
                 fatal, complete, "ECM conservative Type1 selection"),
            Case("pm1-auto-type1", 55050557, 120,
                 ["55050557", "-pm1", "-b1", "191"],
                 [r"\[Backend Auto\] P-1: Aevum selected", r"family=Type1 FFT3161", r"Gerbicz Li.*passed|No P-1|P-1 factor"],
                 fatal, complete, "P-1 conservative Type1 selection"),
        ]
    else:
        cases += [
...
def main() -> int:
    ...
    work = base / f"PrMers-v99.85-validation-{ns.platform}-{stamp}"
    report = base / f"PrMers-v99.85-report-{ns.platform}-{stamp}"
