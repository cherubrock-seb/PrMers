...
__kernel void kernel_ntt_radix5_mm_first(__global ulong* restrict x,
                                   __global ulong* restrict w,
                                    __global ulong* restrict digit_weight,
                                   const uint m)
{
    const uint gid = get_global_id(0);
    const ulong j = gid % (m);
    const ulong base = 5 * (gid - j) + j;
    const ulong off = STAGE5_BASE + 10 * m + 5 * j;

    const ulong W1 = w[off + 0];
    const ulong W2 = w[off + 1];
    const ulong W3 = w[off + 2];
    const ulong W4 = w[off + 3];
    const ulong x0 = modMul(x[base + 0 * m],digit_weight[base + 0 * m]);
    const ulong x1 = modMul(x[base + 1 * m],digit_weight[base + 1 * m]);
    const ulong x2 = modMul(x[base + 2 * m],digit_weight[base + 2 * m]);
    const ulong x3 = modMul(x[base + 3 * m],digit_weight[base + 3 * m]);
    const ulong x4 = modMul(x[base + 4 * m],digit_weight[base + 4 * m]);

    x[base + 0 * m] = modAdd(x0, modAdd(x1, modAdd(x2, modAdd(x3, x4))));
    x[base + 1 * m] = modAdd(x0,
        modAdd(modMul(x1, W1),
        modAdd(modMul(x2, W2),
        modAdd(modMul(x3, W3), modMul(x4, W4)))));
    x[base + 2 * m] = modAdd(x0,
        modAdd(modMul(x1, W2),
        modAdd(modMul(x2, W4),
        modAdd(modMul(x3, W1), modMul(x4, W3)))));
    x[base + 3 * m] = modAdd(x0,
        modAdd(modMul(x1, W3),
        modAdd(modMul(x2, W1),
        modAdd(modMul(x3, W4), modMul(x4, W2)))));
    x[base + 4 * m] = modAdd(x0,
        modAdd(modMul(x1, W4),
        modAdd(modMul(x2, W3),
        modAdd(modMul(x3, W2), modMul(x4, W1)))));
}

__kernel void kernel_inverse_ntt_radix5_mm_last(__global ulong* restrict x,
                                           __global ulong* restrict wi,
                                           __global ulong* restrict digit_invweight,
                                           const uint m)
{
    ...
    const ulong WI1 = wi[off + 0];
    const ulong WI2 = wi[off + 1];
    const ulong WI3 = wi[off + 2];
    const ulong WI4 = wi[off + 3];
    const ulong inv5 = 14757395255531667457UL;

    const ulong x0 = x[base + 0 * m];
    const ulong x1 = x[base + 1 * m];
    const ulong x2 = x[base + 2 * m];
    const ulong x3 = x[base + 3 * m];
    const ulong x4 = x[base + 4 * m];

    x[base + 0 * m] = modMul(modMul(modAdd(x0, modAdd(x1, modAdd(x2, modAdd(x3, x4)))), inv5),digit_invweight[base + 0 * m]);
    x[base + 1 * m] = modMul(modMul(modAdd(x0,
        modAdd(modMul(x1, WI1),
        modAdd(modMul(x2, WI2),
        modAdd(modMul(x3, WI3), modMul(x4, WI4))))), inv5),digit_invweight[base + 1 * m]);
    x[base + 2 * m] = modMul(modMul(modAdd(x0,
        modAdd(modMul(x1, WI2),
        modAdd(modMul(x2, WI4),
        modAdd(modMul(x3, WI1), modMul(x4, WI3))))), inv5),digit_invweight[base + 2 * m]);
    x[base + 3 * m] = modMul(modMul(modAdd(x0,
        modAdd(modMul(x1, WI3),
        modAdd(modMul(x2, WI1),
        modAdd(modMul(x3, WI4), modMul(x4, WI2))))), inv5),digit_invweight[base + 3 * m]);
    x[base + 4 * m] = modMul(modMul(modAdd(x0,
        modAdd(modMul(x1, WI4),
        modAdd(modMul(x2, WI3),
        modAdd(modMul(x3, WI2), modMul(x4, WI1))))), inv5),digit_invweight[base + 4 * m]);
}
