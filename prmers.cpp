...
static const unsigned int crcTable[256] = {
    ...
    0x26d930ac,0x51de003a,0xc8d75180,0xbfddf00e,0x21b4f4b5,0x56b3c423,0xcfba9599,0xb8bda50f,
    ...
    const std::string &timestamp)
{
    ...
    canon << "" << ";";
    canon << "" << ";";
    canon << osName << ";" << osArchitecture << ";" << timestamp;

    std::string key = canon.str();
    unsigned int crc = computeCRC32(key);
    std::ostringstream hex;
    hex << std::uppercase << std::hex << std::setw(8) << std::setfill('0') << crc;

    oss.str(""); oss.clear();
    oss << prefix << ",\"checksum\":{\"version\":1,\"checksum\":\"" << hex.str() << "\"}";
    oss << "}";
    return oss.str();
}
