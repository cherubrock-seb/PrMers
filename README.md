...
## Portable macOS release packages (v99.11)

macOS release builds target macOS 12.0 rather than the runner SDK, bundle GMP/GMPXX, and are published as arm64, x86_64 and universal2 archives. The workflow verifies every Mach-O deployment target and directly loads the packaged Aevum API before a release is created. See `README_V99.11_MACOS_PORTABLE_AEVUM.md`.

## Apple Silicon Aevum compatibility candidate (v99.21)

v99.21 keeps the published v99.9 routing semantics and adds a macOS-only
OpenCL 1.2 compatibility path for Aevum.  Apple builds use a monolithic
`clBuildProgram` source, conservative 64-thread middle work groups, and retain
the stock LDS middle transposes.  Linux, Windows and CUDA retain the original
OpenCL C 2.0 compile/link path.  See
`README_V99.21_APPLE_OPENCL12_CONSERVATIVE_LDS.md` for details.

### Apple GF61 middle-in staging (v99.22)

On Apple OpenCL 1.2, Aevum keeps its LDS middle/tail transposes but decomposes
the GF61 middle-in pipeline into four smaller in-order kernels. This avoids the
Metal pipeline-creation failure without changing Linux, Windows or CUDA paths
and without allocating another transform-sized buffer. See
`README_V99.22_APPLE_GF61_MIDDLEIN_STAGED_LDS.md`.

### Apple GF61 scalar middle-in front end (v99.23)

The v99.22 smoke isolated the remaining Metal failure to the combined GF61
load plus `middleMul2` stage. v99.23 scalarizes only this front end into load,
factor and apply kernels while retaining the stock middle FFT and final LDS
transpose. Existing output/scratch buffers are reused and non-Apple paths are
unchanged. See `README_V99.23_APPLE_GF61_MIDDLEIN_SCALAR_STAGED_LDS.md`.

### Apple GF61 tailSquare two-kernel LDS (v99.24)

After the complete v99.23 GF61 middle-in staged chain was accepted by the M2
OpenCL-to-Metal compiler, the next rejected pipeline was the combined
`tailSquareGF61`.  v99.24 selects the existing `TAIL_KERNELS=3` policy only for
Apple FFT3161: double-wide LDS processing remains, while lines 0 and H/2 are
isolated in `tailSquareZeroGF61`.  Linux, Windows, CUDA and Marin are unchanged.


### Apple GF61 special-tail scalar staged LDS (v99.25)

The v99.24 M2 smoke created the complete staged GF61 middle-in chain but
failed while Metal created `tailSquareZeroGF61`.  On Apple FFT3161 only, the
two exceptional lines (0 and H/2) now use a tiny `2 * SMALL_HEIGHT` GF61
scratch and six simple kernels: load, FFT1, reverse, scalar pair-square, FFT2
and write.  The normal `tailSquareGF61` remains the upstream double-wide LDS
kernel for every other line.  Linux, Windows, CUDA and Marin are unchanged.


### Apple GF61 special-tail radix-stage FFT (v99.26)
See `README_V99.26_APPLE_GF61_TAILZERO_RADIX_STAGED_LDS.md`.


### Apple GF61 special-tail global-shuffle staging (v99.27)
See `README_V99.27_APPLE_GF61_TAILZERO_GLOBAL_SHUFFLE_STAGED.md`.

### Apple GF61 special-tail global reverse staging (v99.28)

The Apple-only GF61 special-tail path now replaces the final Metal-rejected
LDS reverse with an exact global permutation between its existing two tiny
scratch banks.  The pair-square runs in the intermediate bank and the second
reverse returns the data to bank zero.  The main tail, middle, carry,
non-Apple and Marin paths are unchanged.


### Apple GF61 special-tail PairApple argument fix (v99.29)

The Apple staged special-tail path now passes `scratch`, `bufTrigH`, and `bank` explicitly to `tailSquareZeroGF61PairApple`. This avoids overwriting a fixed middle argument when dynamic arguments are bound from position zero. No OpenCL kernel or non-Apple dispatch is changed.


### Apple GF61 special-tail scalar final write (v99.30)

The M2 smoke for v99.29 proved that the corrected PairApple binding works and that PairApple executes far enough to reach final-kernel creation. Apple then rejected the vector final writer. v99.30 replaces only that final writer with one scalar GF61 store per work-item, preserving the exact writeTailFusedLine address formula and requiring no additional buffer. Embedded Aevum is v0.3.25.


### Apple GF61 special-tail direct final copy (v99.31)

The M2 smoke for v99.30 proved that all special-tail arithmetic stages load, including PairApple, but Metal still rejected the final scalar writer. v99.31 reduces that final pipeline to one direct GF61 load/store and launches it twice with scratch/output bases precomputed by the host. No arithmetic, buffer allocation, main-tail LDS path, Marin path, or non-Apple path changes. Embedded Aevum is v0.3.26.


### Apple GF61 normal-line main-tail global staging (v99.32)

The M2 smoke for v99.31 accepted the complete special-line path, including the direct writer, and then rejected the remaining stock double-wide `tailSquareGF61` pipeline. v99.32 stages all non-special GF61 line pairs through scalar load, global ping-pong height FFT, cross-line reverse, scalar pair-square, reverse, and a second height FFT. The existing input and output transform buffers are reused as the two banks, so no transform-sized allocation is added. The stock double-wide LDS kernel remains unchanged for non-Apple platforms. Embedded Aevum is v0.3.27.


### Apple GF61 fftW global staging (v99.33)

The v99.32 M2 smoke loaded the complete special tail, normal-line main tail, and `fftMiddleOutGF61`, then failed only when creating `fftWGF61`. v99.33 embeds Aevum v0.3.28 and stages this final GF61 width transform through an exact scalar middle-out-layout load plus the already validated global radix/twiddle/shuffle sequence. Existing input/output transform buffers are reused; no transform-sized allocation is added and non-Apple paths keep the stock LDS kernel.


### Apple GF61 tailMul global staging (v99.34)

The v99.33 M2 smoke completed the LL-UNSAFE path and loaded the full staged `fftWGF61` pipeline. LL-SAFE then reached the first prepared multiplication and rejected only the stock monolithic `tailMulGF61`. v99.34 embeds Aevum v0.3.29 and replaces that Apple FFT3161 kernel with exact all-line scalar load, global height FFT, direct stock-coordinate pair multiplication, and final height FFT stages. One GF61-only scratch plane is added on Apple; the prepared multiplicand remains read-only. A targeted Apple queue synchronization also removes the transient asynchronous GPU-read checksum mismatch seen before the multiply. Non-Apple paths keep the stock kernel.

### Apple deterministic reads and strict real-iteration smoke (v99.35)

The v99.34 M2 logs marked LL-SAFE as passed despite stopping at `Iter: 0`, before the staged `tailMulGF61` kernels were created, and still printed an atomic GPU checksum mismatch. v99.35 embeds Aevum v0.3.30, replaces Apple checked-read verification with two synchronous full reads compared word-for-word, and makes the macOS smoke require `Iter >= 1` plus the complete LL-SAFE tailMul kernel sequence. No arithmetic kernel changes relative to v99.34.


### Apple queue-marker flush (v99.36)
Explicitly flushes the OpenCL queue after marker enqueue so Apple can submit staged kernels before marker polling.

### Apple-only queue-marker flush (v99.37)

The queue-marker `clFlush` introduced in v99.36 is now compiled only on macOS (`__APPLE__`). Linux, Windows and NVIDIA retain their previous queue behavior. See `README_V99.37_APPLE_ONLY_QUEUE_MARKER_FLUSH.md`.

### Apple large-plan MIDDLE=8 and stock-reverse GF61 multiply (v99.38)

Real M2 testing exposed two independent issues after v99.37: large PRP/P-1
plans stopped while compiling GF31 `fft8Core`, and the staged generic GF61
multiply caused repeated Gerbicz restoration in small P-1. v99.38 embeds Aevum
v0.3.33, inlines the exact GF31 four-point subtransform only for Apple
OpenCL 1.2, and changes the Apple staged multiply to the literal upstream
`reverse -> pairMul -> reverse` ordering. The complete `MIDDLE=8`,
`WIDTH=1024`, `CARRY64` production-kernel matrix is syntax-tested. See
`README_V99.38_APPLE_MIDDLE8_TAILMUL_STOCK_REVERSE.md`.


### Apple GF61 middle-out and exact local-pair multiply (v99.39)

Large Apple `MIDDLE >= 8` plans stage GF61 middle-out through the existing scratch plane. Generic GF61 multiplication now delegates the central pair operation to the original upstream local-memory routines. Other platforms are unchanged.

### Apple scalar GF61 pair multiply and middle-out (v99.40)

The Apple port replaces the rejected local-pair multiply with scalar special/normal pair kernels and splits large-plan GF61 middle-out into five small pipelines.  The existing GF61 scratch allocation is reused; non-Apple paths are unchanged.

## v99.41 — Apple GF61 special-line trig fix

The Apple staged generic multiply now derives both special lines from the
line-zero trig and applies `TAILTGF61` exactly once to H/2.  This is isolated
to Apple OpenCL 1.2; non-Apple paths remain unchanged.

## v99.42 — Apple Aevum P-1 small-factor safety

Aevum P-1 Stage 1 now uses its native `square_mul(x,3)` path, bypassing the
unvalidated Apple generic multiplication.  Gerbicz-Li is temporarily disabled
for this Aevum route because its accumulator still needs a generic register
product.  Apple generic multiplication now fails closed unless explicitly
enabled for diagnostics with `AEVUM_APPLE_UNSAFE_GENERIC_MUL=1`.

## v99.49 — GitHub-exact FFT3161 arithmetic with Apple prepared multiplication

This version abandons the experimental Apple FFT3231 and GF31-only branches.
It is based on GitHub commit `c013d582821af812b1b542b8874d62cede681d68`
and keeps the Aevum computation model unchanged:

- `FFT_TYPE=1` (`FFT3161`);
- both `GF(M31^2)` and `GF(M61^2)`;
- identical FFT selection, weights, carry and P-1 exponentiation;
- Aevum fast3 disabled, with square followed by multiplication by prepared base 3;
- Gerbicz-Li enabled in normal-memory computations;
- original non-Apple branches unchanged.

The macOS-only compatibility change uses Aevum's existing prepared-multiplicand
pipeline.  The invariant multiplicand is completed once with the upstream
`fftHin` step and consumed with the upstream `tailMulLow` kernels for both CRT
components.  This avoids the unvalidated generic Apple GF61 tail multiply
without changing the arithmetic.  Apple small-factor doubling also uses two
scratch buffers to avoid a `restrict` self-alias; non-Apple keeps the original
one-buffer code.

See `README_V99.49_APPLE_GITHUB_EXACT_PREPARED.md`.


## v99.52 — Apple prepared GF61 tailMulLow staging

The v99.51 M2 run completed the staged `fftHinGF61` preparation and then
rejected only the stock monolithic `tailMulGF61` specialization with
`MUL_LOW=1`. v99.52 routes that Apple-only prepared multiplication through the
same exact staged GF61 height FFT and scalar stock-coordinate pair kernels.
The prepared operand stays read-only; FFT3161 GF31+GF61, carry, weights, P-1,
Gerbicz-Li and non-Apple execution remain unchanged.


## v99.53 — Apple GF61 middle-in restrict alias fix

The standalone arithmetic probe showed that v99.52 already failed on pure
`square(5)`, before prepared multiplication. The two Apple staged GF61
middle-in operations that run in place now expose a single data pointer instead
of aliased `restrict` input/output pointers. Aevum is updated to v0.3.47;
non-Apple paths are unchanged.


## v99.60 — macOS Marin default and nonblocking Aevum queue pacing

macOS now selects the Marin `engine::Reg` backend when no backend option is
supplied.  Aevum remains available with the explicit `-aevum` option; Linux
and Windows retain the existing automatic policy.

The v99.59 V-trace prepared-cache refresh is now gated by
`eng->is_aevum_backend()`, so Marin retains its original multiplicand
preparation schedule. Embedded Aevum v0.3.53 also replaces Apple queue-marker
polling with periodic nonblocking `clFlush` submission. The previous marker
policy remains available for A/B diagnosis with
`AEVUM_APPLE_QUEUE_MARKER_WAIT=1`. `AEVUM_APPLE_STAGE_FINISH=1` is still a
separate strict diagnostic and is not enabled by default.

See `README_V99.60_MACOS_MARIN_DEFAULT_APPLE_QUEUE_FLUSH.md`.
