...
static bool enqueue_square_mod_crt_mixed_odd(GpuPrp& g61, GpuPrp& g31, CrtFusedKernels& fk) {
    ...
    const bool use_mixed_precrt_tile7 = parse_bool_env("PRMERS_CRT_MIXED_PRECRT_TILE7", true) &&
    ...
    auto pack_odd_fwd61 = [&]() {
        cl_kernel kpack = use_mixed_pack_tile7 ? fk.mixed_pack_odd_fwd_tile7_61 :
        ...
        const size_t head_global = use_mixed_pack_tile7 ?
            round_up_size(((static_cast<size_t>(row_m) + 6u) / 7u) * 64u, local64) : g_row_m;
        const char* head_name = use_mixed_pack_tile7 ? "crt_mixed_pack_weight_odd_fwd_tile7_61" :
                                (use_mixed_shift_lut ? "crt_mixed_pack_weight_odd_fwd_shift_61" : "crt_mixed_pack_weight_odd_fwd_61");
        enqueue_kernel(g61, kpack, head_global, &local64, "enqueue mixed fused pack+odd61", head_name);
    };
    auto pack_odd_fwd31 = [&]() {
        cl_kernel kpack = use_mixed_pack_tile7 ? fk.mixed_pack_odd_fwd_tile7_31 :
        ...
        const size_t head_global = use_mixed_pack_tile7 ?
            round_up_size(((static_cast<size_t>(row_m) + 6u) / 7u) * 64u, local64) : g_row_m;
        const char* head_name = use_mixed_pack_tile7 ? "crt_mixed_pack_weight_odd_fwd_tile7_31" :
                                (use_mixed_shift_lut ? "crt_mixed_pack_weight_odd_fwd_shift_31" : "crt_mixed_pack_weight_odd_fwd_31");
        enqueue_kernel(g31, kpack, head_global, &local64, "enqueue mixed fused pack+odd31", head_name);
    };
    ...
    auto odd_inv_precrt_coeffhi = [&]() {
        ...
        const size_t tail_global = use_mixed_precrt_tile7 ?
        ...
        enqueue_kernel(g61, ktail, tail_global, &local64,
    ...
    bool crt_defused_fast)
{
    ...
    if (use_crt_mixed_odd) {
        ...
        const bool mixed_precrt_split_possible = mixed_precrt_split_requested && crt_fused.mixed_residues_to_coeffhi &&
                                                 gpu31.bufDigits32 && gpu61.bufWidthMask32;
        const bool mixed_precrt_tile7_enabled = parse_bool_env("PRMERS_CRT_MIXED_PRECRT_TILE7", true) &&
