...
static void enqueue_crt_garner_carry_gpu(GpuPrp& gpu61, GpuPrp& gpu31, const CarryConfig& cfg, bool oneout_digits = false) {
    ...
    if (gpu61.crtLastUnweightPending && items != 32u) {
        items = 32u;
...
static bool enqueue_square_mod_crt_mixed_odd(GpuPrp& g61, GpuPrp& g31, CrtFusedKernels& fk) {
    ...
    const bool use_mixed_tile14 = parse_bool_env("PRMERS_CRT_MIXED_TILE14", true) &&
                                !parse_bool_env("PRMERS_CRT_MIXED_SHIFT_LUT", false) &&
                                odd == 9u;
    const bool use_mixed_precrt_tile14 = use_mixed_tile14 &&
                                         use_mixed_precrt_coeffhi &&
                                         fk.mixed_odd_inv_precrt_coeffhi_tile14 != nullptr;
    const bool use_mixed_precrt_tile7 = !use_mixed_precrt_tile14 &&
    ...
    const bool use_mixed_pack_tile14 = use_mixed_tile14 &&
    ...
    auto pack_odd_fwd61 = [&]() {
        cl_kernel kpack = use_mixed_pack_tile14 ? fk.mixed_pack_odd_fwd_tile14_61 :
        ...
        const size_t head_global = use_mixed_pack_tile14 ?
        ...
        const char* head_name = use_mixed_pack_tile14 ? "crt_mixed_pack_weight_odd_fwd_tile14_61" :
        ...
        enqueue_kernel(g61, kpack, head_global, use_mixed_pack_tile14 ? &local128 : &local64, "enqueue mixed fused pack+odd61", head_name);
    };
    auto pack_odd_fwd31 = [&]() {
        cl_kernel kpack = use_mixed_pack_tile14 ? fk.mixed_pack_odd_fwd_tile14_31 :
        ...
        const size_t head_global = use_mixed_pack_tile14 ?
            ...
            (use_mixed_pack_tile7 ?
            round_up_size(((static_cast<size_t>(row_m) + 6u) / 7u) * 64u, local64) : g_row_m);
        const char* head_name = use_mixed_pack_tile14 ? "crt_mixed_pack_weight_odd_fwd_tile14_31" :
        ...
        enqueue_kernel(g31, kpack, head_global, use_mixed_pack_tile14 ? &local128 : &local64, "enqueue mixed fused pack+odd31", head_name);
    };
    ...
    auto lds_fwd61 = [&](cl_uint len, cl_uint radix) {
        cl_kernel k = (radix == 512u && mixed_lds512_opt && fk.fwd_lds_stage_61_512opt) ? fk.fwd_lds_stage_61_512opt : fk.fwd_lds_stage_61;
        cl_uint arg = 0;
        ...
        enqueue_kernel(g61, k, groups * local64, &local64, "enqueue mixed LDS fwd61", radix == 512u ? "crt_mixed_lds512_forward_61" : "crt_mixed_lds_forward_61");
    };
    auto lds_fwd31 = [&](cl_uint len, cl_uint radix) {
        cl_kernel k = (radix == 512u && mixed_lds512_opt && fk.fwd_lds_stage_31_512opt) ? fk.fwd_lds_stage_31_512opt : fk.fwd_lds_stage_31;
        cl_uint arg = 0;
        ...
        const size_t groups = (static_cast<size_t>(storage) / static_cast<size_t>(len)) * static_cast<size_t>(len / radix);
        enqueue_kernel(g31, k, groups * local64, &local64, "enqueue mixed LDS fwd31", radix == 512u ? "crt_mixed_lds512_forward_31" : "crt_mixed_lds_forward_31");
    };
    auto lds_inv61 = [&](cl_uint base_len, cl_uint radix) {
        cl_kernel k = (radix == 512u && mixed_lds512_opt && fk.inv_lds_stage_61_512opt) ? fk.inv_lds_stage_61_512opt : fk.inv_lds_stage_61;
        cl_uint arg = 0;
        ...
        enqueue_kernel(g61, k, groups * local64, &local64, "enqueue mixed LDS inv61", radix == 512u ? "crt_mixed_lds512_inverse_61" : "crt_mixed_lds_inverse_61");
    };
    auto lds_inv31 = [&](cl_uint base_len, cl_uint radix) {
        cl_kernel k = (radix == 512u && mixed_lds512_opt && fk.inv_lds_stage_31_512opt) ? fk.inv_lds_stage_31_512opt : fk.inv_lds_stage_31;
        cl_uint arg = 0;
    ...
    auto center_lds512_61 = [&]() {
        cl_uint arg = 0;
        set_karg_mem(fk.mixed_lds512_pair61, arg, g61.bufField, "set mixed lds512 center61 a");
        set_karg_mem(fk.mixed_lds512_pair61, arg, g61.bufTwFwd, "set mixed lds512 center61 twf");
        set_karg_mem(fk.mixed_lds512_pair61, arg, g61.bufTwInv, "set mixed lds512 center61 twi");
        set_karg(fk.mixed_lds512_pair61, arg, pow2_n, "set mixed lds512 center61 pow2_n");
        set_karg(fk.mixed_lds512_pair61, arg, odd, "set mixed lds512 center61 odd");
        set_karg(fk.mixed_lds512_pair61, arg, flags61, "set mixed lds512 center61 flags");
        const size_t blocks = static_cast<size_t>(row_m >> 9);
        const size_t pair_blocks = (blocks >> 1u) + 1u;
        enqueue_kernel(g61, fk.mixed_lds512_pair61, static_cast<size_t>(odd) * pair_blocks * local64, &local64,
                       "enqueue mixed lds512 center61", "crt_mixed_lds512_center_61");
    };
    auto center_lds512_31 = [&]() {
        cl_uint arg = 0;
        set_karg_mem(fk.mixed_lds512_pair31, arg, g31.bufField, "set mixed lds512 center31 a");
        set_karg_mem(fk.mixed_lds512_pair31, arg, g31.bufTwFwd, "set mixed lds512 center31 twf");
        set_karg_mem(fk.mixed_lds512_pair31, arg, g31.bufTwInv, "set mixed lds512 center31 twi");
        set_karg(fk.mixed_lds512_pair31, arg, pow2_n, "set mixed lds512 center31 pow2_n");
        set_karg(fk.mixed_lds512_pair31, arg, odd, "set mixed lds512 center31 odd");
        set_karg(fk.mixed_lds512_pair31, arg, flags31, "set mixed lds512 center31 flags");
        const size_t blocks = static_cast<size_t>(row_m >> 9);
        const size_t pair_blocks = (blocks >> 1u) + 1u;
        enqueue_kernel(g31, fk.mixed_lds512_pair31, static_cast<size_t>(odd) * pair_blocks * local64, &local64,
                       "enqueue mixed lds512 center31", "crt_mixed_lds512_center_31");
    };
    ...
    auto odd_inv_precrt_coeffhi = [&]() {
        ...
        cl_kernel ktail = use_mixed_shift_lut ? fk.mixed_odd_inv_precrt_coeffhi_shift :
        ...
        const size_t tail_global = use_mixed_precrt_tile14 ?
            round_up_size(((static_cast<size_t>(row_m) + 13u) / 14u) * 128u, local128) :
            (use_mixed_precrt_tile7 ?
            round_up_size(((static_cast<size_t>(row_m) + 6u) / 7u) * 64u, local64) :
            (use_mixed_precrt_outpar ? g_storage : g_row_m));
        const char* tail_name = use_mixed_shift_lut ? "crt_mixed_odd_inv_precrt_coeffhi_shift" :
        ...
        enqueue_kernel(g61, ktail, tail_global, use_mixed_precrt_tile14 ? &local128 : &local64,
    ...
    bool crt_defused_fast)
{
    ...
    if (use_crt_mixed_odd) {
        ...
        const bool mixed_precrt_split_possible = mixed_precrt_split_requested && crt_fused.mixed_residues_to_coeffhi &&
                                                 gpu31.bufDigits32 && gpu61.bufWidthMask32;
        const bool mixed_tile14_enabled = parse_bool_env("PRMERS_CRT_MIXED_TILE14", true) &&
        ...
        const bool mixed_precrt_tile14_enabled = mixed_tile14_enabled && mixed_precrt_possible;
        const bool mixed_precrt_tile7_enabled = !mixed_precrt_tile14_enabled &&
                                                 parse_bool_env("PRMERS_CRT_MIXED_PRECRT_TILE7", true) &&
        ...
        const bool mixed_pack_tile14_enabled = mixed_tile14_enabled && !mixed_shift_lut_enabled &&
                                             crt_fused.mixed_odd_pack_tile14_available();
        const bool mixed_pack_tile7_enabled = !mixed_pack_tile14_enabled &&
                                             parse_bool_env("PRMERS_CRT_MIXED_PACK_TILE7", true) &&
...
static Options parse_args(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        } else if (arg == "--crt-center" || arg == "--crt-local-square" || arg == "--crt-lds-square") {
            if (i + 1 >= argc) throw std::runtime_error("missing value after " + arg);
